...
<script>
    ...
    function showAdd(){
        location.href="/project/after/add_goods.jsp";
    }
    function checkAll(){
        var goodsids=document.getElementsByName("goodsid");
        if(allnum == 0){
    ...
    function delectAll(){
        if(confirm("确定删除吗？")){
            ...
            window.location.href="/project/DeleteGoodsServlet?goodsid="+ids;
        }
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse"
            ...
            <ul class="nav navbar-nav">
                ...
                <li class="active"><a href="<%=path%>/UserFuzzyServlet?pageNo=1">用户管理</a></li>
            ...
            <ul class="nav navbar-nav navbar-right">
                <li><a href="<%=path%>/after/login.jsp">${name}</a></li>
    ...
    <div class="row">
        <div class="col-md-6">
            <button type="button" onclick="showAdd()" id="addBtn"
                    class="btn btn-info">添加商品</button>
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-primary"
                                ...
                                data-content="${goods.comment}！">弹详情</button>
                    </td>
...
<div class="container">
    <nav aria-label="..." class="navbar-right" style="margin-right: 15px">
        <ul class="pagination">
            ...
            %>
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo-1}&goodsname=${goodsname}&deployDate=${deployDate}&typeId=${typeId}"
            ...
            <li><a class="btn"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo+1}&goodsname=${goodsname}&deployDate=${deployDate}&typeId=${typeId}">${pageNo}</a></li>
            <li><a class="btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo+1}&goodsname=${goodsname}&deployDate=${deployDate}&typeId=${typeId}"
            ...
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo}&goodsname=${goodsname}&deployDate=${deployDate}&typeId=${typeId}">${pageNo}</a></li>
            <li class="disabled"><a href="#" aria-label="Previous">»<span
                    class="sr-only">(current) </span></a></li>
            <%
                }
            %>
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageCount}&goodsname=${goodsname}&deployDate=${deployDate}&typeId=${typeId}&typeId=${typeId}"
